        ...
        <div class="bg-white rounded-2xl shadow-md p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="modalidadeCadastro" class="w-full border rounded-lg px-3 py-2">
                ...
                <div>
                    ...
                    <select id="faixaCadastro" class="w-full border rounded-lg px-3 py-2" disabled>
                ...
                <div>
                    ...
                    <select name="id_graduacao" id="grauCadastro"
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody>
            ...
            <tr class="border-b hover:bg-gray-50 transition linha-graduacao"
                ...
                <td class="py-3 px-4">{{ $det->graduacao->gradu_grau }}</td>
                <td class="py-3 px-4">{{ $det->graduacao->modalidade->mod_nome }}</td>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ── CADASTRO: modalidade → faixa → grau ──────────────────────────
        ...
        modalidadeSelect.addEventListener('change', function() {
            ...
            faixas.forEach(faixa => {
                faixaSelect.innerHTML += `<option value="${faixa}">${faixa}</option>`;
            });
        });
        ...
        faixaSelect.addEventListener('change', function() {
            ...
            graduacoes
                ...
                .sort((a, b) => a.ordem - b.ordem)
                .forEach(g => {
                    grauSelect.innerHTML += `<option value="${g.id}">Grau ${g.grau}</option>`;
                });
        });
        ...
        const modalidadeSalva = localStorage.getItem('graduacao_aluno_modalidade');
        if (modalidadeSalva) filtroModalidade.value = modalidadeSalva;

        filtroModalidade.addEventListener('change', function() {
            localStorage.setItem('graduacao_aluno_modalidade', this.value);
        });
        ...
        function aplicarFiltro() {
            ...
            linhas.forEach(linha => {
                ...
                if (!modalidade) mostrar = false;
                if (modalidade && linha.dataset.modalidade !== modalidade) mostrar = false;
                if (graduacao && linha.dataset.graduacao !== graduacao) mostrar = false;
        ...
        filtroModalidade.addEventListener('change', function() {
            ...
            filtroGraduacao.innerHTML = '<option value="">Todas</option>';
            filtroGraduacao.value = '';

            if (!modalidade) {
                filtroGraduacao.disabled = true;
                filtroGraduacao.innerHTML = '<option value="">Primeiro selecione uma modalidade</option>';
                aplicarFiltro();
                return;
            }
            ...
            linhas.forEach(linha => {
                if (linha.dataset.modalidade === modalidade)
                    disponiveis.add(linha.dataset.graduacao);
            });

            [...disponiveis].sort().forEach(g => {
        ...
        limparBtn.addEventListener('click', function() {
            localStorage.removeItem('graduacao_aluno_modalidade');
        ...
        if (filtroModalidade.value) {
            filtroModalidade.dispatchEvent(new Event('change'));
        } else {
            aplicarFiltro();
        }
