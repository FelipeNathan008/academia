...
<div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-md p-8">
    ...
    <form action="{{ route('horario_treino.update', Crypt::encrypt($horario->id_hora)) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        $diasOld = old('hora_semana');

        if (is_array($diasOld)) {
        $diasSelecionados = $diasOld;
        } else {
        $diasSelecionados = explode(',', $horario->hora_semana);
        }
        @endphp
