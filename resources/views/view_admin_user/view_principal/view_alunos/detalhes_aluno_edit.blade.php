...
<div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-md p-8">
    ...
    <form action="{{ route('detalhes-aluno.update', Crypt::encrypt($detalhe->id_det_aluno)) }}"
        method="POST"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <a href="{{ route('detalhes-aluno.showCertificado', ['path' => Crypt::encrypt($detalhe->det_certificado)]) }}"
                    target="_blank"
        ...
        <div class="flex justify-end gap-4 mt-8">
            <a href="{{ route('detalhes-aluno.index', Crypt::encrypt($aluno->id_aluno)) }}" class="px-5 py-2 border rounded-lg hover:bg-gray-100">Voltar</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const graduacaoAtual =
            "{{ $detalhe->id_graduacao }}";

        const faixaAtual =
            "{{ $detalhe->graduacao->gradu_nome_cor }}";
        ...
        modalidadeSelect.addEventListener('change', function() {
            ...
            faixas.forEach(faixa => {
                ...
                faixaSelect.innerHTML += `
                <option value="${faixa}" ${selected}>
                    ${faixa}
                </option>
            `;
            });

            faixaSelect.dispatchEvent(
                new Event('change')
            );
        });
        ...
        modalidadeSelect.dispatchEvent(
            new Event('change')
        );
    });
